...
import { DollarSign, Check, X, ArrowLeft, Loader2, SearchX, ChevronsUpDown } from "lucide-react";
...
export default function RelComissaoVenda() {
  ...
  const limpar = () => {
    ...
    setDataEntregaInicio(""); setDataEntregaFim(""); setCliente(""); setVendedor("todos");
    setSituacao("todos"); setCentroCusto("todos"); setConsiderarFrete("sim"); setConsiderarDevolucoes(false);
  ...
  const gerar = async () => {
    ...
    try {
      let query = supabase
        .from("ordem_servico")
        .select("id, numero_os, cliente_nome, data_entrada, status, valor_total, vendedor, criado_por")
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Relatório de comissão por venda</h1>
          <p className="text-xs text-muted-foreground">Início &gt; Relatórios de vendas &gt; Comissão de vendedores</p>
        </div>
      </div>
