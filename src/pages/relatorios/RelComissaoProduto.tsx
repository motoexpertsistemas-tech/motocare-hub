...
import { Package, Check, X, ArrowLeft, Loader2, SearchX, ChevronsUpDown } from "lucide-react";
...
import {
  ...
  useFuncionariosRelatorio, useClientesRelatorio, getDefaultDates,
} from "@/hooks/useRelatorioVendasFilters";

export default function RelComissaoProduto() {
  ...
  const [considerarFrete, setConsiderarFrete] = useState("sim");
  const [considerarDevolucoes, setConsiderarDevolucoes] = useState(false);
  const [comissaoPercentual, setComissaoPercentual] = useState(false);
  const [comissaoValor, setComissaoValor] = useState(false);
  ...
  const limpar = () => {
    ...
    setDataEntregaInicio(""); setDataEntregaFim(""); setCliente(""); setGrupo("todos"); setProduto("");
    setVendedor("todos"); setSituacao("todos"); setCanal("todos"); setCentroCusto("todos");
    setConsiderarFrete("sim"); setConsiderarDevolucoes(false); setComissaoPercentual(false); setComissaoValor(false);
  ...
  const gerar = async () => {
    ...
    try {
      let query = supabase
        .from("ordem_servico")
        .select("id, numero_os, cliente_nome, data_entrada, status, valor_total, valor_total_pecas, canal_venda, vendedor, criado_por")
      ...
      if (canal !== "todos") {
        const canalMap: Record<string, string[]> = {
          ...
          os: ["os", "Presencial", "Ordem de Serviço"],
      ...
      toast.success(`${filtered.length} registros de comissão encontrados!`);
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Relatório de comissão por produto</h1>
          <p className="text-xs text-muted-foreground">Início &gt; Relatórios de vendas &gt; Comissão por produto</p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Produto</Label>
              <Input placeholder="Digite para buscar" value={produto} onChange={(e) => setProduto(e.target.value)} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Considerar frete</Label>
              <Select value={considerarFrete} onValueChange={setConsiderarFrete}>
                <SelectTrigger><SelectValue /></SelectTrigger>
          ...
          <div className="flex flex-wrap gap-6 pt-1">
            <label className="flex items-center gap-2 text-sm">
              <Checkbox checked={comissaoPercentual} onCheckedChange={(v) => setComissaoPercentual(!!v)} />
              Comissão por percentual %
            </label>
            <label className="flex items-center gap-2 text-sm">
              <Checkbox checked={comissaoValor} onCheckedChange={(v) => setComissaoValor(!!v)} />
              Comissão por valor R$
            </label>
          </div>
      ...
      {semResultados && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-lg font-semibold text-muted-foreground">Nenhuma comissão encontrada</p>
