...
import { Clock, Check, X, ArrowLeft, ChevronsUpDown } from "lucide-react";
...
export default function RelSituacaoTempo() {
  ...
  const { data: clientes = [] } = useQuery({
    queryKey: ["clientes_rel_sit_tempo"],
    queryFn: async () => {
  ...
  const { data: funcionarios = [] } = useQuery({
    queryKey: ["funcionarios_rel_sit_tempo"],
    queryFn: async () => {
  ...
  const { data: situacoes = [] } = useQuery({
    queryKey: ["situacoes_os_rel_sit_tempo"],
    queryFn: async () => {
  ...
  const { data: formasPgto = [] } = useQuery({
    queryKey: ["formas_pagamento_rel_sit_tempo"],
    queryFn: async () => {
  ...
  const limpar = () => {
    ...
    setVendedor("todos"); setTecnico("todos"); setSituacao("todos");
    setFormaPagamento("todos"); setTransportadora(""); setCentroCusto("todos");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Relatório de situação x tempo</h1>
          <p className="text-xs text-muted-foreground">Serviços / OS &gt; Relatórios &gt; Situação X Tempo</p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          ...
          {/* Situação, Forma pagamento, Transportadora, Centro custo */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-1.5">
