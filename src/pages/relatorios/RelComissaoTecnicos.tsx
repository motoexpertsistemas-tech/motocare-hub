...
import { UserCog, Check, X, ArrowLeft, ChevronsUpDown } from "lucide-react";
...
export default function RelComissaoTecnicos() {
  ...
  const { data: clientes = [] } = useQuery({
    queryKey: ["clientes_rel_comissao_tec"],
    queryFn: async () => {
  ...
  const { data: funcionarios = [] } = useQuery({
    queryKey: ["funcionarios_rel_comissao_tec"],
    queryFn: async () => {
  ...
  const { data: situacoes = [] } = useQuery({
    queryKey: ["situacoes_os_rel_comissao_tec"],
    queryFn: async () => {
  ...
  const limpar = () => {
    ...
    setDataSaidaInicio(""); setDataSaidaFim(""); setCliente(""); setTecnico("todos");
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">Relatório de comissão de técnicos</h1>
          <p className="text-xs text-muted-foreground">Serviços / OS &gt; Relatórios &gt; Comissão de técnicos</p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          ...
          {/* Cliente, Técnico, Situação, Centro de custo */}
