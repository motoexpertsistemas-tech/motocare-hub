...
interface RecebimentoLoteDialogProps {
  ...
  contas: ContaReceber[];
  onConfirmar: (ids: string[], formaPagamento: string, contaBancaria: string, dataRecebimento: string) => void;
}

let nextFormaId = 1;

export function RecebimentoLoteDialog({ open, onOpenChange, contas, onConfirmar }: RecebimentoLoteDialogProps) {
  ...
  const pendentes = contas.filter(c => c.situacao !== "Confirmado" && c.situacao !== "Recebido");

  const filtradas = tab === "cliente"
    ? pendentes.filter(c => clienteBusca && c.entidade.toLowerCase().includes(clienteBusca.toLowerCase()))
    : pendentes.filter(c => {
  ...
  const handleConfirmar = () => {
    if (selectedIds.length === 0) { toast.error("Selecione pelo menos um recebimento"); return; }
    ...
    if (diff > 0.01) { toast.error(`Os valores das formas não batem com o total. Diferença: R$ ${fmt(diff)}`); return; }
    ...
    onConfirmar(selectedIds, formasValidas[0].forma, formasValidas[0].conta, dataRecebimento);

    // Gerar comprovante automaticamente
    printRecibo({
      tipo: "recebimento",
      numero: selecionadas[0]?.numero || 0,
      descricao: selecionadas.length === 1 ? selecionadas[0].descricao : `Lote com ${selecionadas.length} recebimento(s)`,
      ...
      juros: jurosNum,
      dataVencimento: selecionadas[0]?.data || dataRecebimento,
      dataConfirmacao: dataRecebimento,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[95vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Recebimento em Lote</DialogTitle>
        </DialogHeader>

        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="cliente" className="flex items-center gap-2">
              <User className="h-4 w-4" /> Seleção Cliente
            </TabsTrigger>
          ...
          <TabsContent value="cliente" className="space-y-4 mt-4">
            <div className="space-y-1.5">
              <Label>Cliente</Label>
              <ClienteCombobox value={clienteBusca} onChange={setClienteBusca} placeholder="Buscar por #número, nome ou apelido..." />
            </div>
          </TabsContent>
            ...
            {selectedIds.length > 0 && (
              <div className="bg-muted/50 rounded-lg p-4 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{selectedIds.length} recebimento(s) selecionado(s)</span>
                ...
                <div className="space-y-3">
                  ...
                  {formas.map((f, idx) => (
                    <div key={f.id} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_120px_100px_40px] gap-3 items-end border rounded-lg p-3 bg-background">
                      <div className="space-y-1.5">
                        <Label className="text-xs">Forma de recebimento {idx + 1} *</Label>
        ...
        {filtradas.length === 0 && (tab === "multipla" || clienteBusca) && (
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          >
            <Check className="h-4 w-4 mr-1" /> Confirmar Recebimento
          </Button>
        </div>
      </DialogContent>
    </Dialog>
